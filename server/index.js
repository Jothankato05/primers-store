...
const { connectDb, hashPassword, User, App, AppVersion, Session } = require('./database');
...
async function autoSeed() {
  ...
  if (userCount === 0) {
    ...
    await User.create({ username: 'admin', email: 'admin@primers.store', password_hash: hashPassword('admin123'), display_name: 'Primers Admin', role: 'admin', email_verified: true });
    const dev = await User.create({ username: 'demo-dev', email: 'dev@primers.store', password_hash: hashPassword('dev123456'), display_name: 'Primers Group', role: 'developer', email_verified: true });
